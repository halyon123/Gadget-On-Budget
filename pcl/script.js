...
function updateCartDisplay() {
    ...
    if (cartItems && cartTotal) {
        ...
        cart.forEach((item, index) => {
            ...
            itemElement.className = 'cart-item d-flex justify-content-between align-items-center p-2 border-bottom'; // Bootstrap classes
            itemElement.innerHTML = `
                <span>${item.name} - ₹${item.price.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
...
const featuredProducts = [
    products.laptops[0], // HP 14s-dq3037tu
    products.tablets[0], // Samsung Galaxy Tab A7 Lite
    products.smartphones[0], // iQOO Z7 Pro 5G
    products.headphones[0], // OnePlus Nord Buds 3 Pro
    products.speakers[0], // Harman Kardon Onyx Studio 6
    products.smartwatches[0], // Noise ColorFit Pro 4
    products.mice[0] // Logitech M235
];
...
window.onload = function () {
    ...
    if (productsGrid) {
        productsGrid.className = 'products-grid row row-cols-1 row-cols-md-3 g-4'; // Bootstrap grid
            ...
            productCard.innerHTML = `
                <div class="card h-100 product-card">
                    ...
                    <div class="card-body text-center">
                        ...
                        <p class="price card-text text-success fw-bold">₹${product.price.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
    ...
    if (shopProductsGrid && window.location.pathname.includes('shop.html')) {
        shopProductsGrid.className = 'products-grid row row-cols-1 row-cols-md-3 g-4'; // Bootstrap grid
    ...
    // Contact form handling and copy functionality for email and phone
    ...
    if (contactForm) {
        contactForm.addEventListener("submit", function (e) {
            ...
            if (name && email && message) {
                ...
                formMessage.className = "text-success"; // Bootstrap class
                contactForm.reset();
            } else {
                formMessage.textContent = "Please fill in all fields!";
                formMessage.className = "text-danger"; // Bootstrap class
            }
        });
        ...
        if (emailLink) {
            emailLink.addEventListener('click', function (e) {
                e.preventDefault();
                const email = "support@gadgetonbudget.com";
                navigator.clipboard.writeText(email).then(() => {
                    const originalText = emailLink.textContent;
                    emailLink.textContent = "Copied to clipboard!";
                    emailLink.style.color = "#27ae60"; // Match hover color
                    setTimeout(() => {
                        emailLink.textContent = originalText;
                        emailLink.style.color = "#2ecc71"; // Reset to original color
                    }, 2000);
                }).catch(err => {
        ...
        if (phoneLink) {
            phoneLink.addEventListener('click', function (e) {
                ...
                navigator.clipboard.writeText(phone).then(() => {
                    ...
                    phoneLink.textContent = "Copied to clipboard!";
                    phoneLink.style.color = "#27ae60"; // Match hover color
                    setTimeout(() => {
                        phoneLink.textContent = originalText;
                        phoneLink.style.color = "#2ecc71"; // Reset to original color
                    }, 2000);
                }).catch(err => {
    ...
    if (loginForm) {
        loginForm.addEventListener("submit", function (e) {
            ...
            if (email && password) {
                ...
                loginMessage.className = "text-success"; // Bootstrap class
                loginForm.reset();
                setTimeout(() => window.location.href = "index.html", 1000);
            } else {
                loginMessage.textContent = "Please fill in all fields!";
                loginMessage.className = "text-danger"; // Bootstrap class
            }
        });
    }
        ...
        signupForm.addEventListener("submit", function (e) {
            ...
            if (name && email && password && confirmPassword) {
                if (password === confirmPassword) {
                    ...
                    signupMessage.className = "text-success"; // Bootstrap class
                    signupForm.reset();
                    setTimeout(() => window.location.href = "login.html", 1000);
                } else {
                    signupMessage.textContent = "Passwords do not match!";
                    signupMessage.className = "text-danger"; // Bootstrap class
                }
            } else {
